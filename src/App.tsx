...
const PLATFORM_ORDER = ['ios', 'android', 'tvos', 'androidtv', 'firetv', 'samsung', 'lg'];
...
function App() {
	...
	const [selectedVersion, setSelectedVersion] = useState<FrameworkVersion>('v15');
	const [generatedTemplates, setGeneratedTemplates] = useState<PlatformTemplate[]>([]);
	const [copiedState, setCopiedState] = useState<{ type: 'line' | 'group'; platformId: string; groupIndex?: number; lineIndex?: number } | null>(null);
	...
	const isCopied = (
		...
		if (!copiedState || copiedState.type !== type || copiedState.platformId !== platformId) {
			return false;
		}
		if (type === 'group') {
				...
				<Col md={6}>
					<Form.Group>
						...
						<Form.Select
							...
							onChange={(e) => setSelectedVersion(e.target.value as FrameworkVersion)}
						>
					...
					<div className="d-flex flex-wrap gap-2">
						{PLATFORM_ORDER.map((platformId) => {
							...
							return (
								<Button
									key={platformId}
									variant={selectedPlatforms.has(platformId) ? 'primary' : 'outline-primary'}
									size="sm"
										...
										<div key={group.name} className="mb-4">
											<div className="d-flex justify-content-between align-items-center mb-2">
												...
												<Button
													...
													size="sm"
													onClick={() =>
														handleCopyGroup(template.id, group.commands, groupIndex)
													}
												>
											...
											<div className="bg-light p-3 rounded">
												{group.commands.map((command, lineIndex) => (
													...
													>
														...
														>
															{isCopied('line', template.id, groupIndex, lineIndex) ? (
